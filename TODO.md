...
**今の作業状態**: 予定の追加・編集・削除・カテゴリ表示・月絞り込み（終了済み予定の表示/非表示スイッチ含む）・Google Places Autocompleteによる開催場所入力・DynamoDBへの永続化・フォームバリデーション強化まで実装完了。GitHub（`bqyujiyamada-code/schedule-share`, public）→Vercel自動デプロイの本番環境で一通り実機動作確認済み（AWS Amplify Hostingは検討後に不採用とし、後片付けも完了）。アーキテクチャ・環境変数のまとめはArtifactとして作成済み、Git/GitHub作業の汎用手順は`~/git-github-workflow.md`に切り出し済み。

**次にやりたいこと**（詳細は下記「次に着手すべきこと」参照）:
- カテゴリ管理のUI化
- `location.lat`/`lng`を使ったマップ「表示」機能（現状は登録時の座標取得と外部マップ起動のみ）
- モバイル実機・ダークモードの見た目確認（フォームバリデーション強化・終了済み予定表示スイッチ分を含め、実ブラウザでの動作確認は未実施）
...
- `src/components/ScheduleBoard.tsx`: 月ごとの絞り込みUI
  ...
  - 2026-07-13: 月フィルタとは別に「終了済みの予定も表示する」スイッチ（`showPast`state、デフォルトOFF）を追加。月半ばにその月を見る際、月初の終了済み予定をスクロールで避ける煩わしさを解消する目的
    - 判定は`item.endDate < 今日の日付`（日付粒度、時刻は見ない）。複数日の予定は終了日基準なので、今日進行中の予定は消えない
    - スイッチは月移動をまたいでも状態を保持（月ごとにリセットしない）
    - フィルタ後0件かつ月内に元データがある場合は「予定はありません」ではなく「すべて終了済みです。スイッチをオンにすると表示されます」を表示し、データがあるのに空に見える混乱を防止
    - スイッチはM3の見た目に寄せたカスタム実装（`peer-checked`によるトラック/サム制御）。既存のswitch/toggleコンポーネントは無かったため今回新規実装
